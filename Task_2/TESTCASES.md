...
### Проверка некорректных методов:

- Запрос методом `DELETE` или `POST` на `GET` - статус 405.
...
- Для невалидного `sellerID` (например, строка, None, отрицательные числа) — статус 400.
...
- Запрос методом `DELETE` на `GET` - статус 405.
...
- Для валидного `id` — статус 200.
- Для невалидного `id` (например, строка, None, отрицательные числа) - статус 400
- Для несуществующего `id` - статус 404.
...
- В случае передачи невалидных данных (например, некорректное имя, цена или sellerID) сервер должен вернуть статус 400.

### Проверка структуры данных при сохранении:

- Ответ на запрос создания объявления с некорректными данными (например, неверный тип для name или price) должен содержать:
